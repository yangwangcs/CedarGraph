 ...
 protected:
  ...
  void WriteRealisticData(LsmEngine* engine, int count, int id_offset = 0) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      // 使用 CedarKey + Descriptor 接口，避免 InlineShortStr 长度限制
      CedarKey key;
      key.SetEntityId(vertex_id);
      key.SetTimestamp(Timestamp(timestamp));
      key.SetColumnId(column_id);
      key.SetEntityType(0);
      key.SetSequence(i);
      
      Descriptor desc = Descriptor::InlineInt(0, value_dist(gen));
      Status s = engine->Put(key, desc, Timestamp(timestamp));
      EXPECT_TRUE(s.ok());
    }
  }
...
TEST_F(SSTIntegrationTest, DefaultFormat) {
  ...
  options.enable_accumulated_flush = false;  // 禁用累积 flush，立即生成 SST
