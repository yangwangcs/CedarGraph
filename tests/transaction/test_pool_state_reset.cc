...
class PoolStateResetTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    data_dir_ = "/tmp/test_pool_state_reset_" +
...
TEST_F(PoolStateResetTest, AbortedTransactionCanBeReused) {
  // Acquire a transaction from the pool
  auto* txn1 = pool_->Acquire(TransactionOptions{});
  ASSERT_NE(txn1, nullptr);
  EXPECT_EQ(txn1->GetState(), TransactionState::kActive);

  // Begin the transaction
  Status s = txn1->Begin();
  ASSERT_TRUE(s.ok()) << s.ToString();

  // Abort it (Cleanup() resets state to kActive for reuse)
  s = txn1->Abort();
  ASSERT_TRUE(s.ok()) << s.ToString();
  ...
  // State must be reset to kActive so Begin() succeeds
  EXPECT_EQ(txn2->GetState(), TransactionState::kActive);

  s = txn2->Begin();
  EXPECT_TRUE(s.ok()) << "Begin() failed on reused pooled transaction: " << s.ToString();
