...
TEST(QueryValidatorTest, ValidWhereWithMatchVariablePasses) {
  GraphSchema schema;
  LabelSchema person;
  person.name = "Person";
  schema.node_labels["Person"] = person;
  QueryValidator validator(&schema);
  ...
  auto where = std::make_shared<WhereClause>();
  where->condition = std::make_shared<ComparisonExpr>(
      ComparisonExpr::EQ,
      std::make_shared<PropertyExpr>("n", "name"),
      std::make_shared<LiteralExpr>(Value("Alice")));
  stmt.clauses.push_back(where);
