...
class SizeTieredCompactionEngine {
 public:
  ...
  void SetSharedFileNumber(std::shared_ptr<std::atomic<uint64_t>> counter) {
    shared_file_number_ = std::move(counter);
  }
 ...
 private:
  ...
  std::shared_ptr<std::atomic<uint64_t>> shared_file_number_;
